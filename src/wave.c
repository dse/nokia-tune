...
int wave_data_chunk(chunk_t* raw_chunkp, chunk_t* raw_out_chunkp, chunk_t* fmt_chunkp) {
     ...
     for (size_t i = 0; i < nsamples; i += 1) {
          ...
          if (WAVE_TYPE == SINE_WAVE) {
               ...
               /* in_data[i] is in [INT32_MIN, INT32_MAX] */
               float f = 1.0 * in_data[i] / INT32_MAX;
               f = f < -1.0 ? -1.0 : f > 1.0 ? 1.0 : f; /* clamp */
               /* f is now in [-1.0, 1.0] */
               f = f * M_PI;
               /* f is now in [-pi, pi] */
               float g = sin(8 * f) * powf(cos(f * 0.5), 16);
               out_data[i] = (int32_t)(g * CLIP * INT32_MAX);
          }
     }
